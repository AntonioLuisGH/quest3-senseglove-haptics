...
namespace SG
{
	...
	public class SG_TrackedHand : MonoBehaviour, IHandPoseProvider, IHandFeedbackDevice
    {
        ...
        // World-space drift correction — set from HandOffsetController
        public static Vector3 WorldDriftOffset_L = Vector3.zero;
        public static Vector3 WorldDriftOffset_R = Vector3.zero;

        protected virtual void UpdateRealHandPose()
        {
            if (this.RealHandSource != null)
            {
                SG_HandPose nextPose;
                if (this.RealHandSource.GetHandPose(out nextPose))
                {
                    l_realHandPose = new SG_HandPose(nextPose);
            ...
            // ── World-space drift correction — applied before poser update ──
            l_realHandPose.wristPosition += this.TracksRightHand()
                ? WorldDriftOffset_R
                : WorldDriftOffset_L;
